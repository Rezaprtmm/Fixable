	...
	useEffect(() => {
		const getPaymentHistory = async function () {
			...
			if (getData.data) {
				...
				const getForm = await axios.post("http://localhost:3001/getreview", { getUsername });
				if (getForm.data) {
		...
		<div className="flex flex-col">
			...
			<div className="flex flex-col mt-[16px] gap-[8px]">
				{historyData.map((item, index) => (
					<OutputTableRev key={`${index}-${item}`} uniqCode={item.uniquecode} id={item._id} overalRate="Good Level" comment={item.message} revDate={item.reviewdate} status="Done" />
				))}
			</div>
			<FooterTable />
		</div>
