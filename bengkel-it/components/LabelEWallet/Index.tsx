...
interface LabelEWalletProps {
	title: string;
	onChange: (value: string) => void;
}

export default function LabelEWallet(props: LabelEWalletProps) {
	const { title, onChange } = props;
	const [isOpen, setIsOpen] = useState(false);
	const [selectedOption, setSelectedOption] = useState("Choose e-wallet");
			...
			<div className="relative mt-[8px]">
				...
				{isOpen && (
					<div className="absolute mt-2 p-2 bg-white border-[1px] border-[#C3C3C3] rounded-[10px] w-full flex flex-col px-[20px] z-10">
						<ButtonDropdown title="OVO" className={`${selectedOption === "OVO" ? "font-bold" : ""}`} onClick={() => handleOptionClick("OVO")} />
						<ButtonDropdown title="Gopay" className={`${selectedOption === "Gopay" ? "font-bold" : ""}`} onClick={() => handleOptionClick("Gopay")} />
					</div>
				)}
