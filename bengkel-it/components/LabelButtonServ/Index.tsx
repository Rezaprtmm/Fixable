...
export default function LabelButtonServ({ title, onClick, selectedCategory, value }: any) {
			...
			<div className="relative mt-[8px]">
				...
				{isOpen && (
					<div className="absolute mt-2 p-2 bg-white border-[1px] border-[#C3C3C3] rounded-[10px] w-full flex flex-col px-[20px] z-10">
						<ButtonDropdown title="Software Repair" className={`${selectedOption === "Software Repair" ? "font-bold" : ""}`} onClick={() => handleOptionClick("Software Repair")} />
						<ButtonDropdown title="Hardware Repair" className={`${selectedOption === "Hardware Repair" ? "font-bold" : ""}`} onClick={() => handleOptionClick("Hardware Repair")} />
						<ButtonDropdown title="Web/App Creation" className={`${selectedOption === "Web/App Creation" ? "font-bold" : ""}`} onClick={() => handleOptionClick("Web/App Creation")} />
					</div>
				)}
