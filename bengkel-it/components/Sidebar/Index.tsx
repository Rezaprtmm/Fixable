...
export default function Sidebar(props: SidebarProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const getData = await axios.post("http://localhost:3001/session", {
        request,
      });
      if (getData.data) {
  ...
  return (
    <aside className="fixed top-0 flex flex-col w-64 h-screen shadow-sidebar">
      <div className="border-b-[1px] border-[#D5D5D5] py-[14px] flex justify-center">
        <Image
          src={"/images/Logo.svg"}
          alt="Logo"
          width={0}
      ...
      <div className="px-[40px] mt-[40px]">
        <Link
          href={"/profile"}
          className={`flex flex-row bg-blue-subtle p-[16px] items-center gap-[16px] rounded-[10px] ${className}`}
        >
          <Image
            ...
            alt="profile"
          />
          <p className="text-blue-main font-poppins text-[16px] font-bold leading-[170%]">
            {username}
          </p>
        </Link>
      </div>
      <div className="flex flex-col justify-center mt-[40px]">
        <p className="pl-[40px] text-[#737373] font-poppins text-[12px] font-normal leading-[170%]">
          Menu
        </p>
