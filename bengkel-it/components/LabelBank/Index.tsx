...
export default function LabelBank(props: LabelBankProps) {
	const { title, onChange } = props;
	const [isOpen, setIsOpen] = useState(false);
	const [selectedOption, setSelectedOption] = useState("Choose Bank");
			...
			<div className="relative mt-[8px]">
				...
				{isOpen && (
					<div className="absolute mt-2 p-2 bg-white border-[1px] border-[#C3C3C3] rounded-[10px] w-full flex flex-col px-[20px] z-10">
						<ButtonDropdown title="BCA" className={`${selectedOption === "BCA" ? "font-bold" : ""}`} onClick={() => handleOptionClick("BCA")} />
						<ButtonDropdown title="Mandiri" className={`${selectedOption === "Mandiri" ? "font-bold" : ""}`} onClick={() => handleOptionClick("Mandiri")} />
					</div>
				)}
