		...
		fetch("http://localhost:3001/checkreserve", {
			...
			.then((data) => {
				if (data == true) {
					...
					setIsReserve(data);
					alert("Reservation ID not found. Make sure you input the correct one.");
				}
			})
			.catch((error) => {
		...
		<div>
			...
			<div className="grid grid-rows-1 ml-64 px-[40px] mt-[40px] pb-[40px]">
				...
				<div className="flex flex-row items-start mt-[14px] gap-[8px]">
					...
					<p className="text-dark-2 font-poppins text-[12px] font-normal leading-[170%]">Enter the Reservation ID first to check your payment information.</p>
				</div>
				<div className="grid mt-[24px] border-b-[1px] border-[#D5D5D5] pb-[24px]">
					<h3 className="text-black font-poppins text-[16px] font-normal leading-[170%]">Reservation ID</h3>
					<div className="grid grid-cols-3 items-center mt-[8px] gap-[20px]">
						<input type="text" className="col-span-2 border-[1px] border-[#f8f8f8] h-[60px] rounded-[10px] shadow-input px-[20px] text-dark-2 font-poppins text-[16px] font-normal leading-[170%]" onChange={handleReserveId} />
						<button className="bg-info-main rounded-[10px] text-white font-poppins text-[16px] font-normal leading-[170%] h-[60px]" onClick={handleSubmit}>
							Check Payment
						</button>
					</div>
				</div>
				<div className="mt-[24px] pb-[50px]">{isReserve ? <CardPayment formCred={formCred} /> : <div> </div>}</div>
			</div>
		</div>
	);
}
