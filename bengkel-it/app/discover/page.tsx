...
export default function Discover() {
	...
	const getUsername = async (userName: any) => {
		...
		const getReserve = await axios.post("http://localhost:3001/getreserve", { userName });
	...
	return (
		<div className="w-full overflow-hidden">
			...
			<div className="ml-[296px] mt-[40px] pr-[40px] pb-[180px]">
				<h3 className="text-black font-poppins text-[22px] font-normal leading-[150%]">Discover</h3>
				<div className="grid grid-cols-1 mt-[40px] bg-blue-subtle p-[40px] rounded-[10px]">
					<h3 className="text-blue-main font-poppins text-[40px] font-normal leading-[130%]">Welcome, {username} 👋</h3>
					<p className="text-blue-darker font-poppins text-[20px] font-normal leading-[170%] mt-[16px]">Have a nice day! Is there anything we can help you with today?</p>
				</div>
				{activeForm.map((item, index) => (
					<div className="grid grid-cols-1 mt-[24px] p-[20px] border-[1px] border-warning-darker rounded-[10px]">
						<h3 className="text-black font-poppins text-[16px] font-normal leading-[170%]">Active Service</h3>
						<p className={`${percentProgress} font-poppins text-[12px] font-normal leading-[170%] italic`}>64% completed</p>
						<div className="grid grid-cols-6 mt-[24px] items-start gap-[5px]">
							<StatusBar title="Reservation" desc1="Reservation accepted" className2="hidden" className3="hidden" ispaid={true} review={""} />
							<StatusBar title="Appointment" desc1="Appointment approved" className2="hidden" className3="hidden" ispaid={true} review={""} />
							<StatusBar title="Meeting" desc1="Meeting completed" className2="hidden" className3="hidden" ispaid={true} review={""} />
							<StatusBar title="Working Process" desc1="In queue" desc2="In progress" desc3="Complete" className3="text-dark-4" ispaid={true} review={""} />
							<StatusBar title="Payment" desc1="Payment paid" className2="hidden" className3="hidden" ispaid={item.ispaid} />
							<StatusBar title="End stage" desc1="Device ready" desc2="Device taken" className1="text-dark-4" className2="text-dark-4" className3="hidden" ispaid={item.ispaid} />
						</div>
					</div>
				))}
